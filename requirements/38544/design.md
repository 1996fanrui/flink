...
├── tryRequestBuffer()     // 非阻塞请求 Network Buffer
├── requestBufferBlocking() // 阻塞请求（仅 Phase 2）
├── spillBuffer(Buffer)    // 将过滤后数据 spill 到磁盘
├── replayToBuffer(Buffer) // 从磁盘读取数据到 Network Buffer
├── hasDiskData()          // 是否有待 replay 的磁盘数据
└── close()                // 幂等清理所有临时文件（可重复调用不报错）
```
...
- 临时文件存放在 `IOManager.getSpillingDirectoriesPaths()` 返回的目录下
...
**旧 attempt 文件清理**：
- `SpillingBufferManager` 初始化时扫描 spill 目录，清理同名模式（`channel-state-spill-*`）的旧 attempt 残留文件

### 3.3 Spill 文件格式

采用简单的顺序写入格式（业界通用的 length-prefixed 格式，Flink ChannelStateSerializer 和 Spark UnsafeSorterSpillWriter 均采用此模式）：

```
[4 bytes: buffer data length (big-endian int)]
[N bytes: buffer data]
[1 byte: buffer type flag (0=data buffer, 1=event buffer)]
```

- 使用 `FileChannel` 顺序写入，关闭时 `force(true)` 确保刷盘
- Replay 时按顺序读取，每次读一个 Buffer 的数据
- 读完一个文件后立即删除
- 初始实现不包含压缩。理由：channel state 恢复是短暂的一次性过程，spill 文件生命周期极短（秒级），压缩/解压的 CPU 开销相对于磁盘 I/O 收益有限，且增加实现复杂度。后续如性能数据显示磁盘 I/O 成为瓶颈，可考虑引入 LZ4 压缩
...
- `SpillingBufferManager` 提供 `createCheckpointIterator()` 方法，返回一个 `CloseableIterator<Buffer>`，将磁盘数据逐个读取为 Network Buffer 供 Checkpoint 使用
- Checkpoint 通过现有的 `ChannelStateWriter.addInputData()` 接口上传
- 注意：此处需要申请 Network Buffer 加载磁盘数据。由于 Checkpoint 时 Task 处于 barrier 对齐阶段，部分 Buffer 可能已释放，内存压力相对可控
- 如果 Checkpoint 触发时内存仍不足，可考虑直接将 spill 文件作为 channel state handle 上传（需要实现自定义的 `StreamStateHandle`），但这是后续优化方向，初始实现采用加载到 Buffer 再上传的方式
...
- `createCheckpointIterator()` 创建时对当前文件列表做快照，并对快照中的文件增加引用计数。持有引用期间 Replay 不删除文件
- `createCheckpointIterator()` 返回 `CloseableIterator<Buffer>`，`close()` 时释放引用计数。Checkpoint 异常时通过 `iterator.close()` 确保引用释放
...
| Spill 文件泄漏（异常退出或 close 未调用） | 中 | 三重保障：1) close() 幂等设计，try-finally 确保调用；2) 文件名包含 attemptId，重启时清理旧 attempt 残留；3) SpillingBufferManager.close() 强制删除所有文件忽略引用计数 |
