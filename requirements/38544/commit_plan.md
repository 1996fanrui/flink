...
**Reference: SpanningWrapper patterns to reuse:**
- Write via `RandomAccessFile("rw").getChannel()` → `FileChannel`, use `FileUtils.writeCompletely(channel, ByteBuffer)` to handle partial writes
- Read via `FileInputStream` → `BufferedInputStream`
- File creation: round-robin across `spillDirs`, random file name, `file.createNewFile()`
- Directory failover: if IOException on one dir, remove it from array, try next
- No `force(true)` — temp files don't need durability guarantee
