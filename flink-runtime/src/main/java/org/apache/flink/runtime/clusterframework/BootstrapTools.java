...
public class BootstrapTools {
    ...
    public static String getTaskManagerShellCommand(
        ...
        String javaOpts = flinkConfig.get(CoreOptions.FLINK_JVM_OPTIONS);
        if (flinkConfig.get(CoreOptions.FLINK_TM_JVM_OPTIONS).length() > 0) {
            javaOpts += " " + flinkConfig.get(CoreOptions.FLINK_TM_JVM_OPTIONS);
        }
