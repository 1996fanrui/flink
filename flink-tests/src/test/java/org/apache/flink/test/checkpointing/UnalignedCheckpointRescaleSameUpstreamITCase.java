...
/**
 * Integration test for unaligned checkpoint rescaling when a two-input task receives both its left
 * input and right input from the same upstream stream.
 */
...
class UnalignedCheckpointRescaleSameUpstreamITCase {
    ...
    private static final String CO_MAP_NAME = "Co-Map";
    ...
    void testRescaleFromUnalignedCheckpointWithSameUpstream(
        ...
        assertLeftAndRightInputsComeFromSameUpstream(initialJobGraph);
        ...
        try {
            ...
            checkpointPath =
                    waitForCheckpointsWithInflightBuffers(initialJobGraph.getJobID(), miniCluster);
        } finally {
        ...
        assertLeftAndRightInputsComeFromSameUpstream(restoredJobGraph);
    ...
    private static String waitForCheckpointsWithInflightBuffers(
            JobID jobID, MiniCluster miniCluster) throws Exception {
        final CompletableFuture<String> checkpointPath = new CompletableFuture<>();
    ...
    private static void assertLeftAndRightInputsComeFromSameUpstream(JobGraph jobGraph) {
        final JobVertex coMapVertex =
                jobGraph.getVerticesSortedTopologicallyFromSources().stream()
                        .filter(vertex -> CO_MAP_NAME.equals(vertex.getName()))
                        .findFirst()
                        .orElseThrow(
                                () ->
                                        new AssertionError(
                                                "Could not find " + CO_MAP_NAME + " vertex."));
        final List<JobEdge> leftAndRightInputs = coMapVertex.getInputs();
        assertThat(leftAndRightInputs).hasSize(2);
        assertThat(leftAndRightInputs.get(0).getSource().getProducer())
                .isSameAs(leftAndRightInputs.get(1).getSource().getProducer());
    }
